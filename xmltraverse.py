...
root = etree.XML(sampleXML, etree.XMLParser(remove_blank_text=True))

print(etree.dump(root))
#print(etree.tostring(root,pretty_print=True))
#thisisgarbage = etree.QName(root).localname
#print("thisisgarbage:\n",thisisgarbage)
assetArray = root[0][0][0]
#print("assetArray length (should be:3)",len(assetArray))
#print("this is:",etree.QName(assetArray).localname)
...
for item in assetArray.iter(xmlNS+"items"):
    ...
    skuMetaData.append([assetHandle,assetType,assetName,assetWidth,assetHeight,assetFileSize,lastModified,lastModifyUser])
    print("skuMetaData entry added:\r\n",skuMetaData[len(skuMetaData)-1])



    #print(etree.QName(child).localname)
    #print(etree.QName(child).namespace)
    #print(child.tag)
